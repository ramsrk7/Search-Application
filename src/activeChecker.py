...
def checkAccesstime():
    ...
    while True:
        ...
        try:
            for f in all_list:
                atime_c = str(datetime.fromtimestamp(os.path.getatime(f)).strftime('%Y-%m-%d %H:%M:%S'))
                try:
                    atime = main.getATIME(f,'atime')
                except:
                    continue
                print("ATIME:",atime, "FROM DB:",atime_c)
                if str(atime).replace("'","") != atime_c:
                    main.updateATIME(file=f, prop='atime', val=atime_c)
        except:
            continue
        time.sleep(20)
...
if __name__ == "__main__":
    folder_path = "/Users/ram/Desktop/Final-Project/Sample_Files"
